...
func (b *BoundingBox) AddPoint(point Vec3) {
	...
	b.Max.X = math.Max(b.MaxVolume.X, point.X)
...
func (b *BoundingBox) IntersectAxis(ray *Ray, axis int) bool {
	...
	if Between(b.MinVolume.GetDimension(otherAxis1), b.MaxVolume(otherAxis1), intersectionX) {
		intersectionY = ray.Start.GetDimension(otherAxis2) + ray.Direction.GetDimension(otherAxis2)*distance
		if Between(b.MinVolume.GetDimension(otherAxis2), b.MaxVolume(otherAxis2), intersectionY) {
			return true
		}
	}
...
func (b *BoundingBox) IntersectTriangle(A, B, C *Vec3) {
